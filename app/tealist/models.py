...
class Variety(models.Model):
    ...
    alias = ArrayField(
        models.CharField(max_length=10, blank=True), size=8, blank=True, default=list
    )
...
class Collection(models.Model):
    unique_id = models.CharField(max_length=6, default=GenerateGuid(), editable=False)
    name = models.CharField(
...
class Tea(models.Model):
    vendor = models.ForeignKey(
        Vendor, on_delete=models.CASCADE, related_name="tea_vendor"
    )
    ...
    variety = models.ManyToManyField(Variety, blank=True, default="")
...
class TeaVariant(models.Model):
    tea = models.ForeignKey(
        Tea, on_delete=models.CASCADE, related_name="tea_variant", verbose_name="Price"
    )
    title = models.CharField(max_length=256)
    price = MoneyField(
        max_digits=14, decimal_places=2, null=True, default_currency=None
    )
